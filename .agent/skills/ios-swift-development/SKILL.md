...
When auditing code, output findings in this format (Description must be in Chinese):

```
file:line - [category] Description of issue in Chinese
```

Example:
```
src/Services/UserService.swift:15 - [type] 缺少返回类型声明
src/Models/Order.swift:42 - [modern] 使用 match 表达式代替 switch
src/Controllers/ApiController.swift:28 - [solid] 类承担了过多的职责
```
## Language

All explanations, documentation, comments, **reasoning steps, analysis**, and communication with the user MUST be in **Chinese (Simplified)**. 
**Generated artifacts (including but not limited to implementation plans, work reports, task lists, and code comments) MUST also be in Chinese (Simplified).**
Code, variable names, and standard technical terms should remain in English.
